...
export default class HumidityRepository implements ICrudHumidity {
    ...
    create = async (data: HumidityDto): Promise<boolean> | never => {
        const result = await pool.query(
            'INSERT INTO humidity ( humidity, location, date) VALUES ($1, $2, to_timestamp($3))',
            [data.humidity, data.location, data.date]
        )
    ...
    async read(filter: any): Promise<HumidityDto[]> {
        const result = await pool.query<HumidityDto>(
            'SELECT * FROM humidity WHERE id = $1',
            [filter]
        )
        return result.rows
    }
    ...
    async updateById(id: string, data: HumidityDto): Promise<HumidityDto> {
        const result = await pool.query<HumidityDto>(
            'UPDATE humidity SET humidity = $1, location = $2, date = to_timestamp($3) WHERE id = $4',
            [data.humidity, data.location, data.date, id]
        )
    ...
    async deleteById(id: string): Promise<boolean> {
        const result = await pool.query('DELETE FROM humidity WHERE id = $1', [
            id,
        ])
        return result.rowCount === 1
    }
