...
export default class TemperatureRepository implements ICrudTemperature {
    ...
    create = async (data: TemperatureDto): Promise<boolean> | never => {
        ...
        const result = await pool.query("INSERT INTO temperature ( temperature, location, date) VALUES ($1, $2, to_timestamp($3))",[data.temperature, data.location, data.date]);
    ...
    async read(filter: string): Promise<TemperatureDto[]> {
        ...
        const result = await pool.query<TemperatureDto>("SELECT * FROM temperature WHERE id = $1",[filter]);
        return result.rows;
    ...
    async deleteById(id: string): Promise<boolean> {
        const result = await pool.query("DELETE FROM temperature WHERE id = $1",[id]);
        return result.rowCount === 1;
    };

    createTable = async () => {
        const result = await pool.query(`CREATE TABLE IF NOT EXISTS temperature(  
