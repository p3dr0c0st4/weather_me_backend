...
export default class HumidityController {
    ...
    getHumidity = async (req: Request, res: Response, next: NextFunction) => {
        ...
        const tempArr: HumidityDto[] = await this.humidityService.readHumidity()

        // //if success return Array of docs
        return res.status(200).json({ data: tempArr })
    }

    getHumidityById = async (
        req: Request,
        res: Response,
        next: NextFunction
    ) => {
        ...
        //Get  docs from DB
        const temp = await this.humidityService.readById(id)
    ...
    updateHumidity = async (
        ...
        try {
            ...
            let result: HumidityDto = await this.humidityService.updateHumidity(
                id,
                temp
            )
